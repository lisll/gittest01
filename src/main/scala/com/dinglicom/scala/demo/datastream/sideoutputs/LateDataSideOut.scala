...
object LateDataSideOut {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaConfig = ConfigUtils.apply("json") //json格式：{fruit:"apple",idx:1,number:1,time:1538359881000}
  ...
  class CustomWatermarkExtractor extends AssignerWithPeriodicWatermarks[JSONObject] {
    ...
    override def extractTimestamp(element: JSONObject, previousElementTimestamp: Long): Long = {
      ...
      System.out.println("key:"+element.getString("fruit")+",eventtime:["+element.getLong("time")+"|"+sdf.format(element.getLong("time"))+"],currentTimestamp:["+currentTimestamp+"|"+sdf.format(currentTimestamp)+"],watermark:["+getCurrentWatermark().getTimestamp()+"|"+sdf.format(getCurrentWatermark().getTimestamp())+"]")
