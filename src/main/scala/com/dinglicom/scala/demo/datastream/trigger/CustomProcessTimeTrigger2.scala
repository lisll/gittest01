...
/**
  ...
  */
class CustomProcessTimeTrigger2 extends Trigger[(String, Long), TimeWindow] {
  ...
  override def onElement(element: (String, Long), timestamp: Long, window: TimeWindow, ctx: Trigger.TriggerContext): TriggerResult = {
    ...
    // CONTINUE是代表不做输出，也就是，此时我们想要实现比如5条输出一次，
    // 而不是窗口结束再输出就可以在这里实现。
    if (flag > 4) {
      System.out.println("onElement : " + element + "，元素个数为" + flag + "触发计算，保留Window内容")
      flag = 1
      TriggerResult.FIRE
    }
    else {
      System.out.println("onElement : " + element + "，元素:" + flag)      
      flag += 1
